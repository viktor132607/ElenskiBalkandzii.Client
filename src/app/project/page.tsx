const categories = [
  {
    ...
    items: ["Свинско месо", "Кюфтета и кебапчета", "Наденички"],
  },
  {
    id: "mezeta",
    title: "Мезета",
    ribbon: "bg-[#0b9c4a] text-white",
    items: ["Суджуци и луканки", "Сушени меса", "Мезе плата"],
  },
  {
    id: "sirena",
    title: "Сирена",
    ribbon: "bg-[#d8b66b] text-[#2b211c]",
    items: ["Бяло сирене", "Кашкавал", "Сирена за плато"],
  },
];
          ...
          <div key={category.id} id={category.id} className="mb-[72px] scroll-mt-[220px]">
            ...
            <div className="grid grid-cols-3 gap-[18px] max-[1100px]:grid-cols-2 max-[820px]:grid-cols-1">
              {category.items.map((title, index) => (
                <article
                  key={title}
                  className={`flex min-h-[180px] items-center border border-[#e4ddd7] border-t-[6px] bg-white p-7 max-[820px]:min-h-[140px] ${index === 0 ? "border-t-[#cf2428]" : index === 1 ? "border-t-[#0b9c4a]" : "border-t-[#d6b36d]"}`}
                >
